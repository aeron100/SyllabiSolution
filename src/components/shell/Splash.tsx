...
import { TileLink } from '../ui/TileLink';
 ...
 * the hero on first load, with the stacking-pages motif, one sentence, a
 * link to the directions PDF, and Get started. A native modal <dialog>, so
 * the page behind is inert and Escape closes it; focus starts on Get started
 * so Enter or Escape both carry a returning user straight on. Following the
 * link also closes it: the PDF opens in a new tab and the app is ready when
 * the user comes back. Nothing is remembered, so it shows on every load.
 */
export function Splash({ directionsHref, onDismiss }: SplashProps) {
  ...
  return (
    ...
    >
      ...
      <div className="splash-actions">
        <TileLink variant="primary" size="lg" icon="bi-file-earmark-pdf" href={directionsHref} newTab onClick={onDismiss}>
          {SPLASH.directions}
        </TileLink>
      ...
      <p className="sg-hint">{SPLASH.hint}</p>
    </dialog>
  );
}
